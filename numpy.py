...
class Array:
        ...
        if isinstance(data, list):
            if all(isinstance(i, list) for i in data):
                ...
                if len(data) > 0 and len(data[0]) > 0 and all(len(row) == len(data[0]) for row in data):
                    ...
                    raise ValueError("Toutes les lignes doivent avoir le même nombre d'éléments")
            elif all(isinstance(i, (int, float)) for i in data):
                # 1D array
                self.data = [data]
                self.shape = (len(data),)
            else:
                raise ValueError("Liste d'entrée invalide")
        else:
    ...
    def __add__(self, other: Union['Array', int, float]) -> 'Array':
        if isinstance(other, Array):
            ...
            raise TypeError("Type non pris en charge pour l'addition")
    ...
    def __sub__(self, other: Union['Array', int, float]) -> 'Array':
        if isinstance(other, Array):
            ...
            raise TypeError("Type non pris en charge pour la soustraction")
    ...
    def __mul__(self, other: Union['Array', int, float]) -> 'Array':
        if isinstance(other, Array):
            ...
            raise TypeError("Type non pris en charge pour la multiplication")
    ...
    def __truediv__(self, other: Union['Array', int, float]) -> 'Array':
        if isinstance(other, Array):
            ...
            raise TypeError("Type non pris en charge pour la division")
